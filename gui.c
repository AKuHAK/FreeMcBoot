...
void Play_Sound(void)
{
    ...
    for (i = 0; i < SAMPLES_TICK; i++)
    {
        ...
        if ((snd_pos + i) <= (nSizeSample / 2))
        {
            ...
            // if (((snd_pos + i) * 2 + 1) <= (nSizeSample / 2))
            // {                                                     // stereo
...
void drawString_verdana(u32 x, u32 y, u64 color, const char *string)
{
    ...
    for (i = 0; i < l; i++)
    {
        c = (u8)string[i];
        if (c > 127)
            c = 127; // security check as the font is incomplete
...
void draw_dialog(int alpha_dialog, int alpha_control)
{
    ...
    for (i = 0; i < MAX_DIALOG_LINES; i++)
    {
        if ((dialog_buffer[i] != NULL) && (strlen(dialog_buffer[i]) > 0))
        {
            ...
            drawString_verdana(((
                                    SCREEN_WIDTH - getStringWidth_verdana(msg)) /
                                2),        // X
                               y,          // Y
                               color_text, // color
                               msg);       // string
    ...
    if (internal_dialog_type == DIALOG_OK)
    {
        ...
        if (selected_dialog_button == 0)
            selected_dialog_button = 1;
    ...
    else if (internal_dialog_type == DIALOG_YES_NO)
    {
        ...
        if (selected_dialog_button == 1)
            ...
            drawString_verdana(x1, y, color_unselected, msg1);

        if (selected_dialog_button == 2)
            drawString_verdana(x2, y, color_selected, msg2);
        else
            drawString_verdana(x2, y, color_unselected, msg2);
    }
...
void draw_up_panel(int y, int selected_button, int highlight_pulse, int highlight_blw)
{
    ...
    // Draw delimiters
    for (i = 1; i < 6; i++)
    { // skip 1st and last up panel delimiters
...
void load_Textures(void)
{
    ...
    if ((pPng = pngOpenRAW(&background, size_background)) > 0)
    { // tex size = 0x140000
    ...
    if ((pPng = pngOpenRAW(&bar_up, size_bar_up)) > 0)
    { // tex size = 0x8000
        if ((pImgData = malloc(pPng->width * pPng->height * (pPng->bit_depth / 8))) > 0)
        {
            if (pngReadImage(pPng, pImgData) != -1)
    ...
    if ((pPng = pngOpenRAW(&bar_down, size_bar_down)) > 0)
    { // tex size = 0x8000
        if ((pImgData = malloc(pPng->width * pPng->height * (pPng->bit_depth / 8))) > 0)
        {
            if (pngReadImage(pPng, pImgData) != -1)
    ...
    if ((pPng = pngOpenRAW(&bar_delimiter, size_bar_delimiter)) > 0)
    { // tex size = 0x4000
        if ((pImgData = malloc(pPng->width * pPng->height * (pPng->bit_depth / 8))) > 0)
        {
            if (pngReadImage(pPng, pImgData) != -1)
    ...
    if ((pPng = pngOpenRAW(&option_install_normal, size_option_install_normal)) > 0)
    { // tex size = 0x4000
        if ((pImgData = malloc(pPng->width * pPng->height * (pPng->bit_depth / 8))) > 0)
        {
            if (pngReadImage(pPng, pImgData) != -1)
    ...
    if ((pPng = pngOpenRAW(&option_install_multi, size_option_install_multi)) > 0)
    { // tex size = 0x8000
        if ((pImgData = malloc(pPng->width * pPng->height * (pPng->bit_depth / 8))) > 0)
        {
            if (pngReadImage(pPng, pImgData) != -1)
    ...
    if ((pPng = pngOpenRAW(&option_launch_fmcb, size_option_launch_fmcb)) > 0)
    { // tex size = 0x4000
        if ((pImgData = malloc(pPng->width * pPng->height * (pPng->bit_depth / 8))) > 0)
        {
            if (pngReadImage(pPng, pImgData) != -1)
    ...
    if ((pPng = pngOpenRAW(&option_fmcb_cfg, size_option_fmcb_cfg)) > 0)
    { // tex size = 0x8000
        if ((pImgData = malloc(pPng->width * pPng->height * (pPng->bit_depth / 8))) > 0)
        {
            if (pngReadImage(pPng, pImgData) != -1)
    ...
    if ((pPng = pngOpenRAW(&option_format_mc, size_option_format_mc)) > 0)
    { // tex size = 0x4000
        if ((pImgData = malloc(pPng->width * pPng->height * (pPng->bit_depth / 8))) > 0)
        {
            if (pngReadImage(pPng, pImgData) != -1)
    ...
    if ((pPng = pngOpenRAW(&option_uninstall, size_option_uninstall)) > 0)
    { // tex size = 0x8000
        if ((pImgData = malloc(pPng->width * pPng->height * (pPng->bit_depth / 8))) > 0)
        {
            if (pngReadImage(pPng, pImgData) != -1)
    ...
    if ((pPng = pngOpenRAW(&credits_coded, size_credits_coded)) > 0)
    { // tex size = 0x14000
        if ((pImgData = malloc(pPng->width * pPng->height * (pPng->bit_depth / 8))) > 0)
        {
            if (pngReadImage(pPng, pImgData) != -1)
    ...
    if ((pPng = pngOpenRAW(&credits_gui, size_credits_gui)) > 0)
    { // tex size = 0x8000
        if ((pImgData = malloc(pPng->width * pPng->height * (pPng->bit_depth / 8))) > 0)
        {
            if (pngReadImage(pPng, pImgData) != -1)
    ...
    if ((pPng = pngOpenRAW(&highlight, size_highlight)) > 0)
    { // tex size = 0x4000
        if ((pImgData = malloc(pPng->width * pPng->height * (pPng->bit_depth / 8))) > 0)
        {
            if (pngReadImage(pPng, pImgData) != -1)
    ...
    if ((pPng = pngOpenRAW(&highlight_bw, size_highlight_bw)) > 0)
    { // tex size = 0x4000
        if ((pImgData = malloc(pPng->width * pPng->height * (pPng->bit_depth / 8))) > 0)
        {
            if (pngReadImage(pPng, pImgData) != -1)
    ...
    if ((pPng = pngOpenRAW(&font_verdana, size_font_verdana)) > 0)
    { // tex size = 0x20000
        if ((pImgData = malloc(pPng->width * pPng->height * (pPng->bit_depth / 8))) > 0)
        {
            if (pngReadImage(pPng, pImgData) != -1)
