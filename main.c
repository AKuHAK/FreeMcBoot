...
int load_X_modules(void) // Loads XSIO2MAN, XMCMAN, XMCSERV, XPADMAN
{
    ...
    if (((p = strrchr(launch_path, '/')) == NULL) && ((p = strrchr(launch_path, '\\')) == NULL))
...
int MemoryCard_Check(void)
{
    ...
    if ((ret0 != 0) && (ret1 != 0))
    {
        ...
        do
        {
            ...
            if (r == 1)
            { // mc function finished
                if (check_mc == 0)
                {
                    check_cnt++;
                    if (check_cnt == 2)
                    { // Wait 2nd check before to switch to other mc
                        if (ret == 0)
                        {
                            port = 0;
        ...
        if ((files[i].stats.attrFile == MC_ATTR_norm_file) || (files[i].stats.attrFile == MC_ATTR_prot_file))
        {
            ...
            for (j = mc_files; j < (mc_files + src_files); j++)
            {
                if (!strcmp(file_path, files_install[j].path_installed))
                {
                    if (j == mc_files)
                        cnf_found = 1;
                    else if (j == mc_files + 1)
                        fmcb_cfg_elf_found = 1;
    ...
    for (i = 0; i < nfiles; i++)
    {
        ...
        {                                                                                       // check that file is a file
            if ((!strncmp(files[i].name + (strlen(files[i].name) - strlen(".ELF")), ".ELF", 4)) // Check if file extension is .elf
                || (!strncmp(files[i].name + (strlen(files[i].name) - strlen(".ELF")), ".elf", 4)) || (!strncmp(files[i].name + (strlen(files[i].name) - strlen(".ELF")), ".Elf", 4)))
            {
                // fmcb_needed_space += files[i].stats.fileSizeByte;
                fmcb_needed_space += (files[i].stats.fileSizeByte + 1024 - (files[i].stats.fileSizeByte % 1024));
                num_files++;
    ...
    switch (rom_ver[4])
    {
        case 'H':
            ...
            for (i = 0; i < NUM_DUMMIES_FILES; i++)
                memset(dummies_tab[i] + 2, 'A', 1);
            break;
        case 'J':
            for (i = 0; i < NUM_OSD_FILES; i++)
                memset(install_tab[i] + 2, 'I', 1);
            for (i = 0; i < NUM_DUMMIES_FILES; i++)
                memset(dummies_tab[i] + 2, 'I', 1);
            break;
        case 'U':
            for (i = 0; i < NUM_OSD_FILES; i++)
                memset(install_tab[i] + 2, 'A', 1);
        ...
        case 'E':
            ...
            for (i = 0; i < NUM_DUMMIES_FILES; i++)
                memset(dummies_tab[i] + 2, 'E', 1);
            break;
        default:
            for (i = 0; i < NUM_OSD_FILES; i++)
                memset(install_tab[i] + 2, rom_ver[4], 1);
            for (i = 0; i < NUM_DUMMIES_FILES; i++)
                memset(dummies_tab[i] + 2, rom_ver[4], 1);
            break;
    }
...
int Install_Hack(int port) // Copy all needed files on mc0:
{
    ...
    switch (romver[4])
    {
        ...
        default:
            ...
            for (i = 0; i < NUM_DUMMIES_FILES; i++)
                memset(dummies_tab[i] + 2, romver[4], 1);
            break;
    }
    ...
    if (!install_with_dummies)
    {
        ...
        if (!OSDname)
            OSDname = "osdmain.elf"; // if not found default to 'osdmain.elf'
    ...
    {
        if (copy_err == 0)
            log_result[log_index] = 1; // done
        else
            log_result[log_index] = 0; // failed
        log_index++;
        Update_GUI();
        if (copy_err)
            return 0;
...
int Run_Hack(int port) // Run the hack : generates mcid, inject dvdelf...
{
    ...
    if (!File_Exist(tmp))
    { // Test if BOOT.ELF is existing in INSTALL folder
    ...
    */
    ...
    if (mc_Free < 1024)
        sprintf(tmp1, "Memory Card %1d Free Space: %d KB", port + 1, mc_Free);
    else if (mc_Free >= 1024)
        sprintf(tmp1, "Memory Card %1d Free Space: %.1f MB", port + 1, (float)mc_Free / 1024);

    log_job_buffer[log_index] = tmp1;
    log_index++;
    Update_GUI();

    if ((fmcb_space >= 0) && (fmcb_space < 1024))
        sprintf(tmp2, "FMCB install will take: %d KB", fmcb_space);
    else if ((fmcb_space >= 0) && (fmcb_space >= 1024))
        sprintf(tmp2, "FMCB install will take: %.1f MB", (float)fmcb_space / 1024);
    else if ((fmcb_space < 0) && (fmcb_space < 1024))
        sprintf(tmp2, "FMCB install will free: %d KB", 0 - fmcb_space);
    else if ((fmcb_space < 0) && (fmcb_space >= 1024))
        sprintf(tmp2, "FMCB install will free: %.1f MB", 0 - ((float)fmcb_space / 1024));
    ...
    {
        if (filesize < 153600)
            r = embed(decrypted_dvdelf, NULL, 0, bootelf, bootfilesize, injected_dvdelf);
        else
    ...
    {
        if (filesize < 153600)
            r = embed(decrypted_dvdelf, NULL, 0, &launcher2, size_launcher2, injected_dvdelf);
        else
        {
...
int main(int argc, char *argv[])
{
    ...
    if (!strncmp(run_path, "mass0:", 6))
    { // SwapMagic boot path for usb_mass
