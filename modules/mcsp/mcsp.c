...
void MC_OSD_Scan_Thread(void *Data)
{
    ...
    {
        if (port > 1)
            printf("mcsp: Invalid memory card port is specified.\n");
        else
            printf("mcsp: Memory card port is not specified\n");
        goto end;
    }
    ...
    if (version < 0x200)
        printf("mcsp: Using old MCMAN module.\n");
...
void MC_Dummies_Patch_Thread(void *Data)
{
    ...
    for (i = 0; i < Packet->num_entries; i++)
    {
        ...
        if (j > 0)
        {
            if (eraseblocks[j] != eraseblocks[j - 1])
                j++;
        }
        else
            j++;
    ...
    for (i = 0; i < j; i++)
    {
        ...
        if (eraseblocks[i] != 0)
            r = pMcEraseBlock(port, slot, eraseblocks[i], 0);
        if (r != sceMcResSucceed)
        {
            goto end;
        }
        ...
        for (z = 0; z < blocksize; z++)
        {
            ...
            // printf("%10s size:%d cluster:%d ", entry->name, entry->size, entry->entry_cluster);
            if ((eraseblocks[i] * blocksize) + z >= 16)
                r = pMcWritePage(port, slot, (eraseblocks[i] * blocksize) + z, blockbuffer + (z * pagesize), eccbuffer);
            if (r != sceMcResSucceed)
            {
                goto end;
            }
...
void *GetLibraryEntry(char *libname, int version, int entryno)
{
    ...
    for (i = 0; (i < 8) && (libname[i]); i++)
        lib.name[i] = libname[i];
...
void LockMcman()
{
    if (pMcGetIoSema != NULL)
        WaitSema(pMcGetIoSema());
}
//--------------------------------------------------------------
