    ...
    for (port = 0; port < 2; port++)
    {
        if ((state = padGetState(port, 0)) == PAD_STATE_STABLE || (state == PAD_STATE_FINDCTP1))
        {
            ...
            if (ret[port] != 0)
            {
                ...
                if ((padtype_t[port] == 2) && (1 & (test_joy++)))
                { // DualShock && time for joy scan
                    joy_value = 0;
                ...
                { // if no change of pad data
                    n[port]++;
                    if (TV_mode == NTSC)
                    { // Fix repeats for NTSC
                        if (n[port] >= 25)
                        { // NTSC initial repeat delay == 25 loops (0.416 sec)
                    ...
                    { // Fix repeats for PAL
                        if (n[port] >= 21)
                        { // PAL initial repeat delay == 21 loops (0.42 sec)
                            new_pad_t[port] = paddata_t[port];
                            if (nn[port]++ < 20)
...
int setupPad(void)
{
    ...
    for (port = 0; port < 2; port++)
    {
        ...
        { // if anything connected to this port
            modes = padInfoMode(port, 0, PAD_MODETABLE, -1);
            if (modes != 0)
            { // modes != 0, so it may be a dualshock type
                for (i = 0; i < modes; i++)
                {
            ...
            {                        // modes == 0, so this is a digital controller
