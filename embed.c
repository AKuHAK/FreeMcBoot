...
typedef struct
{               // to get start and end chunks for each block
    u8 *start1; // packed
...
u32 FileUnpack(void)
{
    ...
    if (unp.pass1_size < unp.unpacked_size)
    {
        ...
        while (src < endbuf)
        {
            while (r[i].end2 <= src - count && i < b->block_count - 1)
                i++;
            if (r[i].start2 <= src && r[i].start2 > src - count)
                r[i].start3 = dest;
            if (r[i].end2 <= src && r[i].end2 > src - count)
                r[i].end3 = dest;

            if (src[0] != unp.pack_prefix)
            { // just copy data
                *(dest++) = *(src++);
                count     = 1;
            }
...
int inject_data(u8 *launchr2, u8 *bootelf, int l2_insert_before)
{
    ...
    dest = r->start1;
    for (i = 0; i < bufsize; i++)
    { // inject the prepared buffer
        if (i % 30 == 0)
        {
            *(dest++) = '\0';
            *(dest++) = '\0'; // hash value: no bits set
            *(dest++) = '\0';
            *(dest++) = '\0';
        }
        *(dest++) = buf[i];
