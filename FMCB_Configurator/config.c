...
size_t storeFmcbCNF(char *cnf_buf)
{
    ...
    // Launch keys
    for (i = 0; i < 17; i++)
    { // Loop to save the ELF paths for launch keys
...
int saveFmcbCNF(char *fmcbMsg, char *CNF)
{
    ...
    if (strncmp(CNF, "mass:", 5))
    { // if not the mass device
        mcSync(0, NULL, NULL);
    ...
    *fmcb   = *tmpfmcb;
    ...
    while (1)
    {
        ...
        if (readpad())
        {
            ...
            {               // s= 1-2-3 stands matches the 3 launch paths
                event |= 2; // event |= valid pad command
                if (s == 1)
                {
...
cancel_exit:
        ...
        if(event||post_event){ //NB: We need to update two frame buffers per event
            ...
            if (s < SHOW_TITLES) {
                if (swapKeys)
                    sprintf(c, "�1:%s �0:%s �2:%s", LNG(Browse), LNG(Clear), LNG(Edit_Title));
                else
                    sprintf(c, "�0:%s �1:%s �2:%s", LNG(Browse), LNG(Clear), LNG(Edit_Title));
