...
void genLimObjName(char *uLE_path, int reserve)
{
    ...
    if (r > p)
    {                 // if this place is above string start
...
int genFixPath(char *inp_path, char *gen_path)
{
    ...
    if (!strncmp(uLE_path, "mass", 4))
    { // if using USB mass: path
        loadUsbModules();
...
/*int menu(const char *path, const char *file)
{
    ...
    while(1){
        ...
        if(event||post_event){ //NB: We need to update two frame buffers per event
            ...
            if (swapKeys)
                sprintf(tmp, "�1:%s �0:%s", LNG(OK), LNG(Cancel));
            else
                sprintf(tmp, "�0:%s �1:%s", LNG(OK), LNG(Cancel));
            if(sel==PASTE)
                sprintf(tmp+strlen(tmp), " �2:%s", LNG(PasteRename));
            sprintf(tmp+strlen(tmp), " �3:%s", LNG(Back));
...
/*char *PathPad_menu(const char *path)
{
    ...
    while(1){
        ...
        if(event||post_event){ //NB: We need to update two frame buffers per event
            ...
            if(swapKeys) {
                sprintf(textrow, "�1:%s ", LNG(Use));
                if(!setting->PathPad_Lock){
                    sprintf(tmp, "�0:%s �2:%s ", LNG(Clear), LNG(Set));
                    strcat(textrow, tmp);
                }
            } else {
                sprintf(textrow, "�0:%s ", LNG(Use));
                if(!setting->PathPad_Lock){
                    sprintf(tmp, "�1:%s �2:%s ", LNG(Clear), LNG(Set));
                    strcat(textrow, tmp);
                }
            }
            sprintf(tmp, "�3:%s L1/R1:%s", LNG(Back), LNG(Page_leftright));
            strcat(textrow, tmp);
    ...
    while (1)
    {
        ...
        if (readpad())
        {
            ...
            else if ((swapKeys && new_pad & PAD_CROSS) || (!swapKeys && new_pad & PAD_CIRCLE))
            {
                ...
                else if (sel == WFONTS * HFONTS)
                {          //'OK' exit-button selected ?
        ...
        */
        ...
        if (event || post_event)
        { // NB: We need to update two frame buffers per event
            ...
            if (swapKeys)
            {
                sprintf(tmp, "�1:%s �0", "Use");
            }
            else
            {
                sprintf(tmp, "�0:%s �1", "Use");
            }
            sprintf(tmp + strlen(tmp), ":%s �2:%s L1:%s R1:%s START:%s �3:%s",
...
int keyboard2(char *out, int max)
{
    ...
    while(1){
        ...
        if(event||post_event){ //NB: We need to update two frame buffers per event
            ...
            if (swapKeys)
                printXY("�1:OK �0:Back L1:Left R1:Right START:Enter",
                    x, y, setting->color[2], TRUE, 0);
            else
                printXY("�0:OK �1:Back L1:Left R1:Right START:Enter",
                    x, y, setting->color[2], TRUE, 0);
        }//ends if(event||post_event)
        drawScr();
        post_event = event;
    ...
    while(1){
        ...
        if(event||post_event){ //NB: We need to update two frame buffers per event
            ...
            for(i=0, y=mSprite_Y1+FONT_HEIGHT/2; i<menu_ch_h; i++){
                ...
                else if(i==2) sprintf(tmp, "�0 %s", LNG(Filename));
                else if(i==3) sprintf(tmp, "�1 %s%s", LNG(Filename), LNG(_plus_Details));
                else if(i==4) sprintf(tmp, "�2 %s%s", LNG(Game_Title), LNG(_plus_Details));
                else if(i==6) sprintf(tmp, "   %s:", LNG(Sort_Content_by));
                else if(i==7) sprintf(tmp, "   %s", &minuses_s[80-Sort_len]);
                else if(i==8) sprintf(tmp, "�: %s", LNG(No_Sort));
                else if(i==9) sprintf(tmp, "�; %s", LNG(Filename));
                else if(i==10) sprintf(tmp, "�< %s", LNG(Game_Title));
                else if(i==11) sprintf(tmp, "�= %s", LNG(Timestamp));
                else if(i==13) sprintf(tmp, "�3 %s", LNG(Back_to_Browser));
                else tmp[0] = 0;
...
void getFilePath(char *out, int cnfmode)
{
    ...
    while (1)
    {
        ...
        if (readpad())
        {
            ...
            { // Some file is to be selected, not in normal browser mode
        ...
        { // NB: We need to update two frame buffers per event
            ...
            for (i = 0; i < rows; i++) // Repeat loop for each browser text row
            {
                ...
                { // Show normal file/folder names
                    strcpy(tmp, files[top + i].name);
                    if (file_show > 0)
                    { // Does display mode include file details ?
                        name_limit = 43 * 8;
                    }
                    else
                    { // Filenames are shown without file details
                        name_limit = 71 * 8;
                    }
                }
                if (name_limit)
                {                                  // Do we need to check name length ?
                    ...
                    if (strlen(tmp) > name_end)
                    {                            // Is name too long for clean display ?
            ...
            { // if more files than available rows, use scrollbar
            ...
            if (nclipFiles)
            { // if Something in clipboard, emulate LED indicator
            ...
            /*            if(cnfmode==DIR_CNF) {//cnfmode Directory Add Start to select dir
                            if (swapKeys)
                                sprintf(msg1, "�1:%s �0:%s �3:%s �2:", LNG(OK), LNG(Cancel), LNG(Up));
                            else
                                sprintf(msg1, "�0:%s �1:%s �3:%s �2:", LNG(OK), LNG(Cancel), LNG(Up));
                            if(ext[0] == '*')
                                strcat(msg1, "*->");
            ...
            if (cnfmode)
            { // cnfmode indicates configurable file selection
                if (swapKeys)
                    sprintf(msg1, "�1:%s �0:%s �3:%s �2:", "OK", "Cancel", "Up");
                else
                    sprintf(msg1, "�0:%s �1:%s �3:%s �2:", "OK", "Cancel", "Up");
                if (ext[0] == '*')
                    strcat(msg1, "*->");
                strcat(msg1, cnfmode_extU[cnfmode]);
                if (ext[0] != '*')
                ...
                // strcat(msg1, tmp);
                 ...
                 if (swapKeys)
                     sprintf(msg1, "�1:%s �3:%s �0:%s �2:%s L1:%s R1:%s R2:%s Sel:%s",
                         LNG(OK), LNG(Up), LNG(Mark), LNG(RevMark),
                         LNG(Mode), LNG(Menu), LNG(PathPad), LNG(Exit));
                 else
                     sprintf(msg1, "�0:%s �3:%s �1:%s �2:%s L1:%s R1:%s R2:%s Sel:%s",
                         LNG(OK), LNG(Up), LNG(Mark), LNG(RevMark),
                         LNG(Mode), LNG(Menu), LNG(PathPad), LNG(Exit));
             }*/
            setScrTmp(msg0, msg1);
