...
int Read_SYSTEM_CNF(char *boot_path, char *ver)
{
    ...
    {
        // A variable was found, now we dispose of its value.
        if (!strcmp(name, "BOOT2"))
        { // check for BOOT2 entry
...
void load_chkesr_module(void)
{
    ...
    { // first try for Fat PS2 EROMDRV
        for (i = 0; i < 8; i++)
        { // 8 tries for slims PS2 EROMDRVx, to find the valid MG region
...
void launch_osdsys(void) // Run OSDSYS
{
    ...
    if (OSDSYS.hack_enabled)
    {
        r = 0;
        for (i = 0; i < NEWITEMS; i++)
        { // Ckeck in all OSDSYS item array
            for (j = 0; j < 3; j++)
            { // Ckeck all 3 possible path
                if ((OSDSYS.item_path[i][j] != NULL) && (OSDSYS.item_name[i] != NULL))
                { // If name and path not null and empty
                    if ((strlen(OSDSYS.item_path[i][j]) > 0) && (strlen(OSDSYS.item_name[i]) > 0))
                    {
                        ...
                        { // If path isn't set to OSDSYS, check paths
                            if (!file_exists(p_ExecPath))
...
void check_path(void) // check if a path contains FASTBOOT, OSDSYS, B?DATA, or mc?
{
    ...
    if (!strcmp(p_ExecPath, "OSDMENU"))
    { // if path is set to OSDMENU then skip disc boot
        OSDSYS.skip_disc = 1;
        launch_osdsys();
    }
    if (!strncmp(p_ExecPath + 5, "B?DATA-SYSTEM", 13))
...
int FastBoot_Disc(void)
{
    ...
    {
        if (Read_SYSTEM_CNF(cdboot_path, ver) == 1)
    ...
    {
        if (Read_SYSTEM_CNF(cdboot_path, ver) == 2)
        {               // double check PS2 disc type & read CNF
            CleanUp(0); // Cleanup needed
            if (CNF_RAM_p != NULL)
                free(CNF_RAM_p);
...
void SetOsdConfig(void)
{
    ...
    if (!isEarlyJap)
    {                                                                  // Skip SetOsdConfigParam2 for early jap
...
int main(int argc, char *argv[])
{
    ...
    { // if no CNF found, fills p_LK_Path array with default boot paths
        for (i = 0; i < 5; i++)
            for (j = 0; j < 3; j++)
